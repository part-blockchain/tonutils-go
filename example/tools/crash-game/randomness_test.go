...
func TestRandomness(t *testing.T) {
	...
	for roundNumIndex := 0; roundNumIndex <= maxRoundNum; roundNumIndex++ {
		...
		if 0 == roundNumIndex {
			maxRoundNum = int(crashData.RoundNum) + count
		}
		roundNumIndex = int(crashData.RoundNum)
		// 1. 创建新的一轮游戏： new round
		if crashData.GameState == 1 {
			...
			crashData.RoundNum = crashData.RoundNum + 1
		}
		oneRowData.RoundNum = crashData.RoundNum
		...
		// 获取当前游戏的seed和crash乘数
		log.Printf("RoundNum:%d, Crash Seed:%d, CrashMultiple:%d\n", crashData.RoundNum, crashData.Seed, crashData.CrashMultiple)
...
func TestCSV(t *testing.T) {
	...
	GetTonAPIIns()
	maxRoundNum := 100
	csvFile, err := os.Create("crash_multiple.csv")
	defer csvFile.Close()
	// 文件头
	csvwriter := csv.NewWriter(csvFile)
	csvwriter.Write([]string{"RoundNum", "Seed", "CrashMultiple(%)"})
	...
	for roundNumIndex := 0; roundNumIndex <= maxRoundNum; roundNumIndex++ {
		oneRowData := StatisticalData{}
		oneRowData.RoundNum = uint64(roundNumIndex)
		oneRowData.Seed = oneRowData.RoundNum * 2
		oneRowData.CrashMultiple = oneRowData.Seed * 3
		var row []string
		row = append(row, fmt.Sprintf("%d", oneRowData.RoundNum))
		row = append(row, fmt.Sprintf("%d", oneRowData.Seed))
		row = append(row, fmt.Sprintf("%d", oneRowData.CrashMultiple))
